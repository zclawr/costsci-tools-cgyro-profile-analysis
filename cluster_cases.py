...
if __name__== "__main__":
    ...
    num_ion_cases = ion_cases.shape[0]
    num_elec_cases = elec_cases.shape[0]
    ion_mode_file_example = success_files[int(file_idxs[np.array(np.where(omegas > 0))[0,0]])]
    print(f'Number of Electron Mode Cases: {num_elec_cases}')
    print(f'Number of Ion Mode Cases: {num_ion_cases}')
    print('=' * 50)

    kmeans_ion = KMeans(n_clusters=1, init='k-means++', n_init=10, random_state=0).fit(ion_cases[:,:-2])
    ion_centroids = kmeans_ion.cluster_centers_  # Final centroid coordinates
    print(f'Ion Centroids: (rmin, ky, omega)')
    print(ion_centroids)
    print()
    kmeans_elec = KMeans(n_clusters=1, init='k-means++', n_init=10, random_state=0).fit(elec_cases[:,:-2])
    elec_centroids = kmeans_elec.cluster_centers_  # Final centroid coordinates
    print(f'Electron Centroids: (rmin, ky, omega)')
    print(elec_centroids)
    print('=' * 50)
    print('Modes by rmin, ky:')
    
    for i in range(len(rho_values)):
        print(f'rmin={rho_values[i]}:' + '-' * 20)
        for j in range(len(ky_values)):
            print(f'ky={ky_values[j]}:')
            num_elec = np.array(np.where((elec_cases[:,0] == rho_values[i]) & (elec_cases[:,1] == ky_values[j]))).shape[1]
            num_ion = np.array(np.where((ion_cases[:,0] == rho_values[i]) & (ion_cases[:,1] == ky_values[j]))).shape[1]
            print(f'Electron mode cases: {num_elec}')
            print(f'Ion mode cases: {num_ion}')
            print()
        print()
